...
void WeightVector::add(simple_sparse_vector& x, double s) {
	double pred = 0.0, norm_x = 0.0;
	uint N = x.my_vec.size();
	for(uint i = 0; i < N; i++)
	{
		uint first = x.my_vec[i].first;
		uint second = x.my_vec[i].second;
		double val = second*s;
		norm_x += val*val;
		pred += 2.0*my_v[first]*val;
		my_v[first] += (val/my_a);
	}
...
/*
	uint N = u.my_vec.size();
	for(uint i = 0; i < N; i++)
	{
		uint first = u.my_vec[i].first;
		uint second = u.my_vec[i].second;
	    outcome += (second * v[first]);
	}
*/	
	return outcome;
}
